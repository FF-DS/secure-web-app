  ...
  <div class="pa-5">
    <v-dialog v-model="dialog" max-width="600px" min-width="360px">
      <template v-slot:activator="{ on }">
        <v-layout row>
          <v-flex xs6>
            <v-btn x-small text v-on="on" @click="tab = 0" tile class="py-5">
              <v-icon small class="mr-2">mdi-login-variant</v-icon>
              <span v-if="!$vuetify.breakpoint.xsOnly">Login</span>
            </v-btn>
          </v-flex>
          <v-flex xs6>
            <v-btn class="mr-2 py-5" x-small text @click="tab = 1" v-on="on">
              <v-icon small class="mr-2">mdi-account-plus-outline</v-icon>
              <span v-if="!$vuetify.breakpoint.xsOnly">Register</span>
            </v-btn>
          </v-flex>
        </v-layout>
      </template>
        ...
        <v-tabs v-model="tab" show-arrows icons-and-text dense grow>
          ...
          <v-tab-item>
            <v-card class="px-4 mt-1" loader-height="2" :loading="loadingForm">
              <span
                class="ml-5 my-3"
                v-if="getUserError && getUserError.type == 'login'"
              >
                <span v-for="(key, index) in getUserError" v-bind:key="index">
                ...
                <v-form ref="loginForm" v-model="valid" lazy-validation>
                  ...
                  <v-row>
                    <v-col class="d-flex" cols="12">
                      <v-btn small block text color="success" @click="validate"
                        >Login</v-btn
                      >
                    </v-col>
                  </v-row>
          ...
          <v-tab-item>
            <v-card class="px-4 mt-1" loader-height="2" :loading="loadingForm">
              <span
                class="ml-5 my-3"
                v-if="getUserError && getUserError.type == 'register'"
              >
                <span v-for="(key, index) in getUserError" v-bind:key="index">
                  <li class="error-message" v-if="index != 'type'">
                    {{ key }}
